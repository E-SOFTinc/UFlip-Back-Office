...
$("#user_name").blur(function () {
    ...
    var page_location = path_root + "login/load_pass";
    $.post(page_location, {username: username}, function (data) {
        if(data=='yes'){
            location.reload();
        }
    });
...

{
    ...
    // alert("aaaaaggggggggggggggggggggg3333333333333"+b+":"+c);
...
$(document).ready(function ()
{
    ...
    var d = 0;
    document.form.next_1.disabled = false;
    $("#prodcut_id").blur(function () {
        var prdct_id = document.getElementById('prodcut_id').value;
...
//            //check the username exists or not from ajax
// 
//$.post(leg_availability, {sponserleg: $('#position').val(), sponser_user_name: $('#ref_username').val()}, function(data)
//
//            {
//
//                if (trim(data) == 'no') //if username not avaiable
...
//check(a,b,c);
                            ...
                            {
                                msg = "enter more than 6 characters";
                                $(this).removeClass();
                                $(this).addClass('messageboxok');
                                $(this).html('<img align="absmiddle" src="' + path_temp + 'images/Error.png" />' + msg).show().fadeTo(1900, 1);
                                return false;
            ...
            if (data != 'no') {
                ...
                $.post(register_amount_path, function (data2) {
                    ...
                    if (data2 != 'no_data') {
                        ...
                        //$('span.reg_total-title').html(register_amount);
                        var total_reg_amount = parseFloat(data, 10) + parseFloat(data2, 10);
                        hidden.value = total_reg_amount*qty;
                        $('span.total-title').html(total_reg_amount*qty);
                    }
                    else {
...

    {
        ...
        a = 0;
        var pro_status = document.form.pro_status.value;
        var passcode = document.form.passcode.value;
        // alert(passcode);

        var user_nulla = trim(passcode);
        if (pro_status == 'yes')

        {

            var prodcut_id = document.form.prodcut_id.value;
            var user_nullb = trim(prodcut_id);
        }
...

        {
...

            {
                ...
                $("#passbox").removeClass();
                $("#passbox").addClass('messagebox');
                $("#passbox").html('<img align="absmiddle" src="' + path_temp + 'images/Error.png" /> Invalid Product Selection...').show().fadeTo(1900, 1);
                error = 1;
                disable();
            }
...

        {
            ...
            $("#passbox").removeClass();
            $("#passbox").addClass('messageboxerror');
            $("#passbox").html('<img align="absmiddle" src="' + path_temp + 'images/Error.png" /> ' + msg).show().fadeTo(1900, 1);
            error = 1;
...

        {
            ...
            disable();
            var pass_available = path_root + "register/checkPassAvailability";
            //remove all the class add the messagebox classes and start fading
            var msg;
            msg = $("#validate_msg11").html();
            $("#passbox").removeClass();
            $("#passbox").addClass('messagebox');
            $("#passbox").html('<img align="absmiddle" src="' + path_temp + 'images/loader.gif" />' + msg).show().fadeTo(1900, 1);
            //check the username exists or not from ajax
            $.post(pass_available, {prodcutpin: $('#passcode').val(), prodcutid: $('#prodcut_id').val()}, function (data)

            {
...

                {
...

                    {
                        ...
                        $(this).addClass('messageboxok');
                        $(this).html('<img align="absmiddle" src="' + path_temp + 'images/accepted.png" /> ' + msg).show().fadeTo(1900, 1);
                        // inlineMsg('passcode','Invalid passcode',4);
...
//
//
//                    $(this).html('<img align="absmiddle" src="' + path_temp + 'images/accepted.png" />' + msg36).show().fadeTo("fast", 1);
//
//                });
//            }
//            else
