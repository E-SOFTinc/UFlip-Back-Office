	...
	{	
		...
		$image_name3=$_FILES['product_image3']['name'];*/
             //upload product image   
                 ...
                 * create thump
                 */
                /* $source_path = base_url(). 'public_html/images/products/'. $file_name1;
                  $target_path = base_url() . 'public_html/images/products/thumbs/'.$file_name1;
                        $config_manip = array(
                'image_library' => 'gd2',
                 //'library_path'=>'',
		...
		/*if($image_name1=="")
		{
			$thumb_name2 = "imgs/products/thumbs/noimage.png";
			$img_name2 = "imgs/products/noimage.png";
			$img_code1 = 0;
		}
		if($image_name2=="")
		{
			$thumb_name = "images/products/thumbs/noimage.png";
			$img_name = "images/products/noimage.png";
			$img_code2 = 0;
		}
		if($image_name3=="")
		{
		...
		if($image_name1<>"")
		{
//echo "<br>first image session";
			$res1 = $this->checkImage($POST,$name1,'products',1);

			if($res1['err']==0)
			{
				$filename1 = $res1['filename'];
				$image_name1 = $res1['image_name'];
				$extension1 = $res1['extension'] ;
				$status = $this->create_thumb($filename1, $extension1, $image_name1,'product');
                                
				if($status)
				{
					$thumb_name1 = "images/products/thumbs/".$image_name1;
					$img_name1 = "images/products/".$image_name1;
					$img_code1 = 1;
//echo "<br>1:$thumb_name1,$img_name1,$img_code1";
				}
		...
		if($image_name2<>"")
		{
//echo "<br>second image session<br>";
			$res2 = $this->checkImage($POST,$name2,'products',2);
//print_r($res2);
			if($res2['err']==0)
			{
				$filename2 = $res2['filename'];
				$image_name2 = $res2['image_name'];
				$extension2 = $res2['extension'] ;
//echo "<br>Fl Two:$filename2,$image_name2,$extension2";
				$status = $this->create_thumb($filename2, $extension2, $image_name2,'product');
				if($status)
				{
					$thumb_name2 = "images/products/thumbs/".$image_name2;
					$img_name2 = "images/products/".$image_name2;
					$img_code2 = 1;
				//	echo "<br>2:$thumb_name2,$img_name2,$img_code2";
				}
			}
		}
		if($image_name3<>"")
		{
//echo "<br>third image session";
			$res3 = $this->checkImage($POST,$name3,'products',3);
//print_r($res3);
			if($res3['err']==0)
			{
				$filename3 = $res3['filename'];
				$image_name3 = $res3['image_name'];
				$extension3 = $res3['extension'] ;
//echo "<br>Fl Three:$filename3,$image_name3,$extension3";
				$status = $this->create_thumb($filename3, $extension3, $image_name3,'product');
				if($status)
				{
					$thumb_name3 = "images/products/thumbs/".$image_name3;
					$img_name3 = "images/products/".$image_name3;
					$img_code3 = 1;
					//echo "<br>3:$thumb_name3,$img_name3,$img_code3";
				}
			}
		}
	//echo "<br>$img_name1. $thumb_name1,$img_code1, $img_name2, $thumb_name2,$img_code2, $img_name3, $thumb_name3,$img_code3";
		$res = $this->doProduct($POST, $img_name1, $thumb_name1,$img_code1, $img_name2, $thumb_name2,$img_code2, $img_name3, $thumb_name3,$img_code3,'save');
                 * 
                 */
...
function doProduct($POST,$img_name1,$img_name2,$img_name3,$action)
{
    ...
    $res['err']=0;
                ...
                if($action=='update')
		{
                    ...
                    $id=$this->session->userdata('pdt_id');
...
/*function doProduct($POST, $img_name1, $thumb_name1,$img_code1, $img_name2, $thumb_name2,$img_code2, $img_name3, $thumb_name3,$img_code3,$action)
	...
	public function getProducts()
	{
		...
		$res = $this->selectData($selQry,"Error : D11012012T1455PML116");*/
                ...
                $selQry=$this->db->get_where($tbl,array('added_user'=>  $this->session->userdata['logged_in']['user_id']));
		$i=0;
    ...
    function deductPoint($user_id,$point_deduct)
    {
        ...
        $this->db->set("balance_amount",$new_blnce_point);
	...
	public function getProductForUpdate($id)
	{
		...
		$tbl1 = $this->table_prefix.'ecom_products';
                $tbl2 = $this->table_prefix.'ecom_category';
                $tbl3 = $this->table_prefix.'ecom_subcategory';
		$qry = "SELECT $tbl1.id,$tbl1.category_id cid,$tbl2.category as catname,$tbl3.subcategory_name as subcatname,$tbl1.subcategory_id as subcid,product,brand,price,tax,
                featurs,thumb1,description,
                $tbl1.date_of_entry FROM $tbl1 join $tbl2 on $tbl1.category_id= $tbl2.id left join $tbl3 on $tbl1.subcategory_id= $tbl3.subcategory_id where $tbl1.id=$id";
		$res = $this->selectData($qry,'Error 123456789800');
	...
	public function updateProduct($POST)
	{
		...
		if($image_name2=="")
		{
			$id = $_SESSION['pdt_id'];
			$image = $this->getImages($id, 'product',2);
			$img_name2 = $image['image2'];
			$thumb_name2= $image['thumb2'];
			$code2 = $image['img_code2'];
		}
		if($image_name3=="")
		{
			$id = $_SESSION['pdt_id'];
			$image = $this->getImages($id, 'product',3);
			$img_name3 = $image['image3'];
			$thumb_name3= $image['thumb3'];
			$code3 = $image['img_code3'];
		}
		if($image_name2=="")
		{
			$thumb_name = "imgs/products/thumbs/noimage.png";
			$img_name = "imgs/products/noimage.png";
			$img_code2 = 0;
		}
		if($image_name3=="")
		{
		...
		if($image_name2<>"")
		{
			...
			if($res2['err']==0)
			{
				...
				if($status)
				{
					$thumb_name2 = "imgs/products/thumbs/".$image_name2;
					$img_name2 = "imgs/products/".$image_name2;
					$img_code2 = 1;
				}
			}
		}
		if($image_name3<>"")
		{
			...
			if($res['err']==0)
			{
				...
				if($status)
				{
					$thumb_name3 = "imgs/products/thumbs/".$image_name3;
					$img_name3 = "imgs/products/".$image_name3;
					$img_code3 = 1;
				}
			}
		}
		
		$res = $this->doProduct($POST, $img_name1, $thumb_name1,$img_code1, $img_name2, $thumb_name2,$img_code2, $img_name3, $thumb_name3,$img_code3,'update');
	...
	public function deleteProduct($id)
	{
		...
		$code1 = $image1['img_code'];
		$image2 = $this->getImages($id, 'product',2);
		$code2 = $image2['img_code'];
		$image3 = $this->getImages($id, 'product',3);
		$code3 = $image3['img_code'];
		if($code1==1)
		{
			$del_img1 = $image1['image'];
			$del_thumb1 = $image1['thumb'];	
		}
		if($code2==1)
		{
			$del_img2 = $image2['image'];
			$del_thumb2 = $image2['thumb'];	
		}
		if($code3==1)
		{
			$del_img3 = $image3['image'];
			$del_thumb3 = $image3['thumb'];	
		}
		$tbl = $this->table_prefix.'ecom_products';
		...
		if($res)
		{
			...
			$this->deleteImage($del_thumb1);
			$this->deleteImage($del_img2);
			$this->deleteImage($del_thumb2);
			$this->deleteImage($del_img3);
			$this->deleteImage($del_thumb3);
			return 1;
		}
		else
			return 0;*/
