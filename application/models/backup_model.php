...
include_once ( "SqlBackupClass.php" );
$backup = new SqlBackup;
$datetime = date("Y-m-d")."-".date("H")."-".date("i")."-".date("s");
if ( !$backup->setDB ($this->DATABSE_NAME))
...
$file_name_sql="back".date("Y-m-d").".$filetype";
	if ($compression==1) {
		...
		gzclose ($fp);
            ...
            //echo "<br/>".$path.$file_name_sql;
              
		echo "<br/>55555555=".$fp = fopen ("db_backup/dump/".$file_name_sql,"w+");
		fwrite ($fp,$newfile);
		fclose ($fp);
	}
...
<br/>To Download File click here https://infinitemlmsoftware.com/soft/binary/db_backup/dump/$file_name_sql <br/><br/>
	...
	$mailObj->AddAddress('ibrahimkv89@gmail.com'); //PASSWORD:cander1055mlm
	//if($FILE_NAME !="")
