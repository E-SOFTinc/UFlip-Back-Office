...
Class SelectTree  extends Model
{
public $NO_OF_LEVEL; 
...
//echo  $select;
 $res = $this->selectData($select,"Error on select board serial no-3544344236");
 $row = mysql_fetch_array($res);
...
public function getNotSpitedBoardTableNameMaxReached($user_id)
{
 ...
 //echo  $select;
 $res = $this->selectData($select,"Error on select board serial no-35444446");
 $row = mysql_fetch_array($res);
...
public function getBoardID($user_id,$board_table)
{
 ...
 public function getMyBoardIDs($uesr_id,$board_table_no,$user_boar_id_arr=array())
          {
          require_once 'Validation.php';   
       ...
       // print_r($arr_id);
		 ...
		 $m = count($user_boar_id_arr);		 
		  ...
		  // echo "<BR />######".$this->getBoardTopID($row["board_serial_no"],$board_table_no);
				  ...
				  $user_boar_id_arr[$m]["table_no"] =  $board_table_no;
...
public function getBoardIDs($user_ref_id,$board_table)
{
 ...
 public function checkPassCode($prodcutpin)
    {
        require_once 'ProductModel.php';
        $obj_product=new Product();
        $prodcutpin=mysql_real_escape_string($prodcutpin);
       // $productid=mysql_real_escape_string($prodcutid);
        //echo"FDFDF:"$prodcutid:
        //if($obj_product->isProductAvailable($product_id,$prodcutpin))
        return $obj_product->isBoardPassAvailable($prodcutpin);
    }
   public function activateBoardUser($user,$pin)
   {     
		 ...
		 $user_id=$this->obj_vali->UserNameToId($user);
		  ...
		  $check_board=$this->chekUserActivated($user_id);
		  
		   if($pin != "")
		    {
		    $count=$this->chekPin($pin);
		 ...
		 $obj_reg->addBoard($board_no,$user_id,$user);
		 
		 $update_pin=$this->UpdatePin($pin,$user);
    ...
    public function chekPin($pin)
    {
     $qr="SELECT COUNT(*) AS cnt FROM 22pin_numbers WHERE pin_numbers='$pin' AND pin_prod_refid='10' AND status='yes'";
   
    $res= $this->selectData($qr, "Eror on insert 121213232");
   ...
   public function UpdatePin($pin,$user_id)
   {
       $qr = "UPDATE 22pin_numbers SET 
            status='no' , used_user='$user_id' WHERE pin_numbers='$pin'";
	//	echo $qr;	
    $res=$this->updateData($qr, "Errorupdatin pin boad 1233232");
    return $res;
   
   }
   
   public function chekUserActivated($user_id)
   
   {
     $qr="SELECT COUNT(*) AS cnt FROM 22ft_individual WHERE id='$user_id' AND board_activation_status='yes'";
   
  //echo "CHECK". $qr;
    $res= $this->selectData($qr, "Eror on insert 445566+522");
	 ...
	 $row =  mysql_fetch_array($res);
      ...
      $qr= "SELECT date_of_joining FROM $ft_individual WHERE id='$user_id'";
      $res = $this->selectData($qr,"Error on selecting status 23444 from ft");
      $row=mysql_fetch_array($res);
