...
class income_details_model extends Inf_Model {
    ...
    public function userNameToID($posted_user_name) {
        $this->Obj_val->userNameToID($posted_user_name);
        return $posted_user_name;
    }
    
     public function userIdToName($user_id) {
        $user_name = $this->Obj_val->IdToUserName($user_id);
        return $user_name;
    }
    ...
    public function add_income($id) {
        ...
        foreach ($res->result_array() as $row) {
            $view_amt_type = $this->validation->getViewAmountType($row["amount_type"]);
            $array["det$i"]["amount_type"] = $view_amt_type;
            $array["det$i"]["amount_payable"] = $row["amount_payable"];
            $array["det$i"]["user_id"] = $this->userIdToName($row["user_id"]);
    ...
    public function getPermittedMenus($user_id,$url)
    {
       ...
       $this->db->select("id");
       $this->db->from("34_infinite_mlm_menu");
       ...
       else
       {
           ...
           $this->db->from("34_infinite_mlm_sub_menu");
        ...
        //if(isset($menu_id)&&isset($menu_sub_refid))
