...
class Shopping extends Inf_Controller {
    ...
    function my_cart() {
        ...
        $this->set("tran_Back_to_Product_Categories", $this->lang->line('Back_to_Product_Categories'));
        ///////////////////////
        $cart_arr = $this->session->userdata('user_cart');
        $user_id = $this->session->userdata('ecom_user_id');
        if (isset($cart_arr["count"]) && ($cart_arr["count"] > 0)) {
            for ($i = 0; $i < $cart_arr["count"]; $i++) {
                ...
                if ($this->input->post("update_qty" . $i)) {
                    ...
                    $qty = $POST["qty" . $i];
                    $pdt_id = $POST["pdtid" . $i];
                    $this->session->set_userdata("user_product$i", array('qty' => $qty, 'product_id' => $pdt_id));

                    if ($qty <= 0) {
                        $this->session->set_userdata("user_cart", array('count' => $cart_arr["count"] - 1, 'amount' => $this->shopping_model->getTotalAmount($user_id)));
                    }
                }
            }
        }
        ...
        $count = $this->shopping_model->getShoppingInfo($user_id);
        $this->set('items', $count);
        $amount = $this->shopping_model->getTotalAmount($user_id); //die();
        $this->set('amount', $amount);
    ...
    function remove_item($id = '') {
        ...
        if ($result) {
            $path = base_url() . 'user/shopping/my_cart';
    ...
    public function shipping_address() {
        ...
        if ($this->input->post('shipping_address_submit')) {
            $this->session->set_userdata('user_shipping_adress', $this->input->post('shipping_adress'));
    ...
    public function checkout() {
        $this->session->set_userdata("user_checkout", "checkout");
        $msg = '';
        $result = $this->shopping_model->checkout("CreditCard", "new_order");
        //die();
        if ($result) {
            // $ecom_user_id = $this->session->userdata('ecom_aff_id');
            $ecom_user_id = $this->LOG_USER_ID;
            //$this->shopping_model->updateUserStatus($ecom_user_id);
...
//vkp ends  
    ...
    function card_payment() {
        ...
        // $path = "https://localhost/WC/synergy-ecommerce/pay/formit_max.php";
        $this->set('payment_submit_form', $path);

        $country = $this->Shopping->obj_vali->viewCountry();
    ...
    function payment_data_submit() {
        $str = $this->input->post("cardnumber");
        $cnt = strlen($str);
        $POST = $this->input->post();
        if ($cnt == 16) {
    ...
    function shipping_details() {
        ...
        $invoice_no = $this->session->userdata("user_invoice_no");
