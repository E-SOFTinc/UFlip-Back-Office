...
<div id="span_js_messages" style="display:none;">
    ...
    <span id="confirm_msg">{$tran_Sure_you_want_to_Activate_this_Product_There_is_NO_undo}</span>
    <input type="hidden" id="path_root" name="path_root" value="{$PATH_TO_ROOT_DOMAIN}">
</div>

<innerdashes>
    <hdash>
        <img src="{$PUBLIC_URL}images/1337773000_inactive-basketpng.PNG" border="0" />
        &nbsp;&nbsp;&nbsp;&nbsp;{$tran_inactive_product}
		{if $HELP_STATUS}
			<a href="https://infinitemlmsoftware.com/help/inactive-product" target="_blank"><buttons><img src="{$PUBLIC_URL}images/1359639504_help.png" /></buttons></a>
		{/if}
    </hdash>
                ...
                <tbody>
                    {foreach from=$product_details item=v}
                        ...
                        <tr>
                            ...
                            <td>{$pair_value}</td>
                            <td>{$status}</td>
                            <td><a href="javascript:activate_prod({$id})"><img src="{$PUBLIC_URL}images/edit.png" title="Activate {$name}" style="border:none;"></a></td>
                        </tr>
                        {$i=$i+1}
                    {/foreach}
                </tbody>
